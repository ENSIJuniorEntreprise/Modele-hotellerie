        ...
        <div className="relative px-0 md:px-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
            {visibleTestimonials.map((t, idx) => (
              <div 
                key={currentPage + idx} 
                className="bg-white border border-[#e8e2d9]/50 p-10 md:p-14 rounded-[32px] shadow-sm flex flex-col items-center text-center group hover:shadow-2xl transition-all duration-500 animate-fadeIn"
              >
                ...
                <p className="text-[#1a2744]/70 font-light leading-relaxed italic text-lg">"{t.text}"</p>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 max-w-4xl mx-auto pt-10 border-t border-[#e8e2d9]">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center">
              <div className="text-3xl md:text-5xl font-serif text-[#1a2744] mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>{stat.value}</div>
              <div className="text-[10px] font-bold text-[#D1A243] tracking-[0.3em] uppercase">{stat.label}</div>
            </div>
          ))}
        </div>
