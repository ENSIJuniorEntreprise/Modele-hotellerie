    ...
    <div className="relative w-full max-w-5xl mx-auto">
      ...
      <div
        ...
        style={{
          aspectRatio: "16/7",
          borderRadius: "2px",
          border: "1px solid rgba(209,162,67,0.25)",
        }}
      >
        <img
          key={current}
          ...
          className="carousel-slide absolute inset-0 w-full h-full object-cover"
          style={{ "--tx": direction === "right" ? "-2%" : "2%" }}
        />
        <div className="absolute inset-0" style={{ background: "linear-gradient(to top,rgba(10,8,5,0.55) 0%,transparent 55%)" }} />
        <div className="absolute bottom-0 left-0 p-3 sm:p-6 z-10">
          <p style={{ fontFamily: "'Inter',sans-serif", fontWeight: 300, letterSpacing: "0.3em", fontSize: "0.6rem", color: "#D1A243", textTransform: "uppercase" }}>
        ...
        <button
          onClick={prev}
          className="carousel-nav absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 flex items-center justify-center rounded-full text-white"
          style={{ width: "clamp(1.8rem, 4vw, 2.4rem)", height: "clamp(1.8rem, 4vw, 2.4rem)" }}
        >
        ...
        <button
          onClick={next}
          className="carousel-nav absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-20 flex items-center justify-center rounded-full text-white"
          style={{ width: "clamp(1.8rem, 4vw, 2.4rem)", height: "clamp(1.8rem, 4vw, 2.4rem)" }}
        >
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        {slides.map((_, i) => (
          <button
            ...
            className={`dot-pill h-[3px] rounded-full ${i === current ? "active" : ""}`}
            style={{ width: i === current ? "2rem" : "0.45rem" }}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <style>{`
        ...
        .vp-controls { position:absolute; bottom:0; left:0; right:0; padding:clamp(0.75rem,2vw,1.5rem) clamp(0.6rem,2vw,1.25rem) clamp(0.6rem,2vw,1.1rem); background:linear-gradient(to top,rgba(5,5,10,0.88) 0%,transparent 100%); transition:opacity 0.35s ease,transform 0.35s ease; }
        .vp-controls.hidden { opacity:0; transform:translateY(6px); pointer-events:none; }
        .vp-progress { width:100%; height:3px; background:rgba(255,255,255,0.2); border-radius:999px; cursor:pointer; position:relative; margin-bottom:clamp(0.4rem,1.5vw,0.85rem); transition:height 0.2s; }
        ...
        .vp-btn { display:flex; align-items:center; justify-content:center; width:clamp(1.6rem,4vw,2.4rem); height:clamp(1.6rem,4vw,2.4rem); border-radius:50%; border:1px solid rgba(209,162,67,0.3); background:rgba(255,255,255,0.07); backdrop-filter:blur(6px); color:#fff; transition:all 0.2s; cursor:pointer; flex-shrink:0; }
        .vp-btn:hover { background:rgba(209,162,67,0.22); border-color:#D1A243; }
        .vp-btn.play { width:clamp(1.9rem,4.5vw,2.8rem); height:clamp(1.9rem,4.5vw,2.8rem); background:rgba(209,162,67,0.15); border-color:rgba(209,162,67,0.5); }
        .vp-btn.play:hover { background:rgba(209,162,67,0.32); }
        .vp-vol { -webkit-appearance:none; width:clamp(40px,8vw,80px); height:3px; border-radius:999px; outline:none; cursor:pointer; }
        ...
        .vp-time { font-family:'Inter',sans-serif; font-size:clamp(0.55rem,1.5vw,0.72rem); font-weight:300; letter-spacing:0.05em; color:rgba(255,255,255,0.75); }
        .vp-qmenu { position:absolute; bottom:calc(100% + 8px); right:0; background:rgba(10,8,5,0.93); border:1px solid rgba(209,162,67,0.25); border-radius:4px; overflow:hidden; backdrop-filter:blur(12px); min-width:70px; z-index:50; }
        .vp-qitem { display:block; width:100%; padding:0.4rem 0.8rem; font-family:'Inter',sans-serif; font-size:0.72rem; letter-spacing:0.05em; color:rgba(255,255,255,0.7); text-align:center; cursor:pointer; transition:all 0.15s; }
        ...
        .vp-bigplay .ring { width:clamp(3rem,8vw,5rem); height:clamp(3rem,8vw,5rem); border-radius:50%; border:1.5px solid rgba(209,162,67,0.6); background:rgba(0,0,0,0.35); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; transition:all 0.25s; }
      ...
      <div className="vp-wrap" onMouseMove={showControls} onMouseEnter={showControls} onMouseLeave={() => setHovered(false)}>
        ...
        {!playing && (
          <div className="vp-bigplay" onClick={togglePlay}>
            <div className="ring">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#E8C97A"><polygon points="6,3 20,12 6,21" /></svg>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center justify-between gap-1 sm:gap-3">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button className="vp-btn play" onClick={togglePlay}>
                {playing
                  ? <svg width="11" height="11" viewBox="0 0 24 24" fill="#E8C97A"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></svg>
                  : <svg width="11" height="11" viewBox="0 0 24 24" fill="#E8C97A"><polygon points="5,3 19,12 5,21" /></svg>
                }
              </button>
              ...
              <span className="vp-time ml-1 hidden xs:inline">{formatTime(currentTime)}</span>
              <span className="vp-time opacity-40 hidden xs:inline">／</span>
              <span className="vp-time hidden xs:inline">{formatTime(duration)}</span>
            </div>

            <div className="flex items-center gap-1 sm:gap-2">
              <button className="vp-btn" onClick={toggleMute}>
                {muted || volume === 0
                  ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" /><line x1="23" y1="9" x2="17" y2="15" /><line x1="17" y1="9" x2="23" y2="15" /></svg>
                  : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07" /><path d="M19.07 4.93a10 10 0 0 1 0 14.14" /></svg>
                }
              </button>
              <input
...
export default function Galerie() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] font-['Cormorant_Garamond']">
      ...
      {/* ─── HERO ─── */}
      <section className="relative w-full flex items-center justify-center overflow-hidden" style={{ height: "clamp(280px, 55vw, 500px)" }}>
      ...
      <section className="py-10 sm:py-16 px-4 sm:px-[5%]">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="font-['Inter'] text-[#D1A243] uppercase tracking-wider" style={{ fontSize: "clamp(12px, 1.8vw, 17px)" }}>
            Explorer les images
          </p>
          <div className="w-[180px] sm:w-[250px] h-px bg-[#1A2744] mx-auto mt-2" />
          <p className="font-['Inter'] text-[#1a253c] mx-auto mt-2 px-4" style={{ fontSize: "clamp(12px, 1.8vw, 17px)" }}>
            "Du lobby au rooftop, explorez chaque recoin de notre établissement"
          </p>
        </div>
        <GalerieCarousel />
      </section>
      ...
      <section className="py-10 sm:py-16 px-4 sm:px-[5%]">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <div className="w-[180px] sm:w-[250px] h-px bg-[#1A2744] mx-auto mt-2" />
          <p className="font-['Inter'] text-[#1a253c] mx-auto mt-2 px-4" style={{ fontSize: "clamp(12px, 1.8vw, 17px)" }}>
            "Plongez dans l'univers raffiné de notre hôtel à travers cette visite immersive."
          </p>
        </div>
        <HotelVideoPlayer />
      </section>
